...
export function jevDecider(api = createJevClient()) {
  return async (request, signal) => {
    ...
    const criteria = Object.fromEntries(Object.entries(request.candidates).map(([id, action]) => [id,
      ...
      action.op === 'press' ? `${JSON.stringify(action)} — ${action.key === 'ArrowDown' ? 'Move to the next suggestion (or open the suggestions). This highlights, but does not commit, an option.' : action.key === 'ArrowUp' ? 'Move to the previous suggestion. This highlights, but does not commit, an option.' : action.key === 'Enter' ? 'Commit the currently highlighted suggestion, or submit this search, as appropriate for the observed widget.' : 'Dismiss the open popup.'}` :
