...
try{
 ...
 for(const mode of ['click','enter']){
  ...
  try{
   const path=join(root,mode+'.json');let code=0;
   try{await exec(process.execPath,[join(skill,'scripts/run.mjs'),'--binary',binary,'--session',session,'--url',origin+'/'+mode,'--intent',autocompleteIntent+(mode==='click'?'':' Use keyboard selection for BOTH fields; commit the highlighted match with '+(mode==='tab'?'Tab':'Enter')+'.'),'--value','contactQuery=adobe','--value','accountQuery=software','--output',path],{timeout:150000,maxBuffer:3e6});}catch(e){code=e.code;}
   const result=JSON.parse(await readFile(path));
   // Publish synthetic observations/actions, never a resumable invocation or local paths.
   entry.result={returnReason:result.returnReason,actions:result.actions,decisions:result.decisions,latestObservation:result.latestObservation,totalMs:result.totalMs};
   assert.equal(code,0);assert.equal(result.returnReason,'reported_complete');
   entry.independentSnapshot=(await browser(['snapshot'])).snapshot;
   assert.match(entry.independentSnapshot,/Selection confirmed/);
   assert.deepEqual(events,[{type:'confirm',selected:autocompleteExpected}]);
   assert.ok(!result.actions.some(a=>a.action.op==='select'));
   if(mode!=='click'){assert.ok(result.actions.some(a=>a.action.op==='press'&&a.action.key==='ArrowDown'));assert.ok(result.actions.some(a=>a.action.op==='press'&&a.action.key===(mode==='tab'?'Tab':'Enter')));}
 ...
 await new Promise(r=>server.close(r));await rm(root,{recursive:true,force:true});report.cleanup.serverClosed=true;report.cleanup.temporaryDirectoryRemoved=true;
