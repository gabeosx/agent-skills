...
async function helper(args,label){const path=join(root,label+'.json');let stdout,code=0;try{({stdout}=await exec(process.execPath,[join(skill,'scripts/run.mjs'),...args,'--output',path],{env:{...process.env,OPENROUTER_API_KEY:key},timeout:150000,maxBuffer:3e6}));}catch(e){stdout=e.stdout;code=e.code;}return {path,code,summary:JSON.parse(stdout),result:JSON.parse(await readFile(path,'utf8'))};}
...
function record(entry,run){(entry.runs??=[]).push(run.result);assert.equal(run.summary.observation.fresh,true);assert.equal(run.summary.observation.snapshot,run.result.latestObservation.snapshot);}
...
await check('input-request-and-resume',async e=>{const first=await helper(startArgs('/workflow',workflowIntent),'input');record(e,first);assert.equal(first.result.returnReason,'input_required');assert.equal(first.code,2);assert.equal(events.filter(x=>x.type==='save').length,0);const second=await helper(['--resume',first.path,...addValues],'input-resumed');record(e,second);assert.equal(second.code,0);await verifyWorkflow(e);assert.equal(events.filter(x=>x.type==='start').length,1);});
await check('budget-pause-and-resume',async e=>{const first=await helper([...startArgs('/workflow',workflowIntent),...addValues,'--max-actions','5'],'budget');record(e,first);assert.equal(first.result.returnReason,'action_budget');const second=await helper(['--resume',first.path,'--max-actions','30'],'budget-resumed');record(e,second);assert.equal(second.code,0);await verifyWorkflow(e);assert.equal(events.filter(x=>x.type==='start').length,1);});
await check('keyboard-search-and-back',async e=>{const r=await helper([...startArgs('/keyboard','Search guides for the supplied query, open the Recovery guide, then use browser back to return to Guides.'),'--value','query=recovery'],'keyboard');record(e,r);assert.equal(r.code,0);e.independentSnapshot=(await browser(['snapshot'])).snapshot;assert.match(e.independentSnapshot,/heading "Guides"/);assert.ok(r.result.actions.some(x=>x.action.op==='press'));assert.ok(r.result.actions.some(x=>x.action.op==='back'));assert.deepEqual(events,[{type:'search',query:'recovery'}]);});
await check('scroll-to-reveal-control',async e=>{const r=await helper(startArgs('/scroll','Scroll down to load the Continue button, activate it, and stop when Updates complete is visible.'),'scroll');record(e,r);assert.equal(r.code,0);assert.ok(r.result.actions.some(x=>x.action.op==='scroll'));assert.deepEqual(events,[{type:'continue'}]);assert.match((await browser(['snapshot'])).snapshot,/Updates complete/);});
}finally{try{await browser(['close']);report.cleanup.browserClosed=true;}catch{report.cleanup.browserClosed=false;}await new Promise(r=>server.close(r));await rm(root,{recursive:true,force:true});report.cleanup.serverClosed=true;report.cleanup.temporaryDirectoryRemoved=true;report.finishedAt=new Date().toISOString();report.verdict=report.cases.length===5&&report.cases.every(x=>x.verdict==='passed')&&report.cleanup.browserClosed?'passed':'failed';await writeFile(output,JSON.stringify(report,null,2));console.log(JSON.stringify({verdict:report.verdict,output}));if(report.verdict!=='passed')process.exitCode=1;}
