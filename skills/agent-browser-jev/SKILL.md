...
- Autocomplete text is a search query, not a committed selection. Select the matching suggestion by click or keyboard and check the selected value before continuing. For keyboard-only dropdowns, have the caller control and verify the selection directly: autonomous Jev keyboard selection is not reliable in current tests. Tab is not offered.
